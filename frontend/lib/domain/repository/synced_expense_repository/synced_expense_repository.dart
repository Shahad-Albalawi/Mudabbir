import 'package:dartz/dartz.dart';
import 'package:mudabbir/data/local/expense_hive_cache.dart';
import 'package:mudabbir/data/network/failure.dart';
import 'package:mudabbir/data/remote/expense_api_service.dart';
import 'package:mudabbir/domain/models/expense_sync_result.dart';
import 'package:mudabbir/domain/models/expense_transaction.dart';
import 'package:mudabbir/domain/repository/expense_repository/expense_repository.dart';
import 'package:mudabbir/domain/services/sync_policies.dart';
import 'package:mudabbir/domain/services/sync_flush_lock.dart';
import 'package:mudabbir/domain/services/repository_guard.dart';
import 'package:mudabbir/presentation/resources/strings_manager.dart';
import 'package:mudabbir/data/network/api_exception.dart';
import 'package:mudabbir/service/getit_init.dart';
import 'package:mudabbir/utils/api_session.dart';

/// Offline-first sync between Laravel expenses API, Hive cache, and SQLite.
class SyncedExpenseRepository {
  final ExpenseRepository _local;
  final ExpenseApiService _remote;
  final ExpenseHiveCache _cache;

  SyncedExpenseRepository({
    ExpenseRepository? local,
    ExpenseApiService? remote,
    ExpenseHiveCache? cache,
  })  : _local = local ?? getIt<ExpenseRepository>(),
        _remote = remote ?? getIt<ExpenseApiService>(),
        _cache = cache ?? getIt<ExpenseHiveCache>();

  Future<Either<Failure, List<Map<String, dynamic>>>> getExpenseCategories() =>
      _local.getExpenseCategories();

  Future<Either<Failure, List<Map<String, dynamic>>>> getAccounts() =>
      _local.getAccounts();

  Future<ExpenseListSyncResult> getTransactions({
    String type = 'expense',
    String? monthKey,
    int? categoryId,
    bool recurringOnly = false,
  }) {
    return guardSyncedOperation(() async {
      final cached = _cache.getExpensesList();

      if (!await hasApiSession()) {
        final local = await _local.getTransactions(
          type: type,
          monthKey: monthKey,
          categoryId: categoryId,
          recurringOnly: recurringOnly,
        );
        return ExpenseListSyncResult(expenses: local.getOrElse(() => []));
      }

      try {
        await flushPendingOps();
        final remote = await _remote.getExpenses();
        await _pruneFromServerSnapshot(remote);
        await _local.mergeServerExpenses(remote);
        await _cache.saveExpensesList(remote.map((e) => e.toJson()).toList());

        final local = await _local.getTransactions(
          type: type,
          monthKey: monthKey,
          categoryId: categoryId,
          recurringOnly: recurringOnly,
        );
        return ExpenseListSyncResult(
          expenses: local.getOrElse(() => []),
        );
      } on ApiException catch (e) {
        if (e.isNetworkError) {
          if (cached != null) {
            return ExpenseListSyncResult(
              expenses: _filterCached(
                cached,
                type: type,
                monthKey: monthKey,
                categoryId: categoryId,
                recurringOnly: recurringOnly,
              ),
              fromCache: true,
              isOffline: true,
            );
          }

          final local = await _local.getTransactions(
            type: type,
            monthKey: monthKey,
            categoryId: categoryId,
            recurringOnly: recurringOnly,
          );
          return local.fold(
            (_) => const ExpenseListSyncResult(expenses: [], isOffline: true),
            (data) => ExpenseListSyncResult(
              expenses: data,
              fromCache: true,
              isOffline: true,
            ),
          );
        }
        rethrow;
      }
    }, fallbackMessage: AppStrings.expensesLoadFailed);
  }

  Future<Either<Failure, ExpenseWriteSyncResult>> addTransaction(
    ExpenseTransaction transaction, {
    bool allowOverBudget = false,
  }) async {
    if (transaction.type == 'expense') {
      final validation = await _validateExpense(
        transaction,
        allowOverBudget: allowOverBudget,
      );
      if (validation != null) return Left(validation);
    }

    return guardRepository(() async {
      try {
        final remote = await _remote.createExpense(transaction.toJson()..remove('id'));
        await _applyServerExpense(remote);

        final budget = await _local.getBudgetSnapshot(
          accountId: remote.accountId,
          date: remote.date,
        );

        return ExpenseWriteSyncResult(
          result: ExpenseWriteResult(
            transactionId: remote.id,
            budgetSnapshot: budget,
            budgetMessage: budget == null
                ? null
                : AppStrings.expensesBudgetLinked(
                    budget.spentAmount,
                    budget.budgetAmount,
                    budget.remaining,
                  ),
          ),
        );
      } on ApiException catch (e) {
        if (!e.isNetworkError) throw RepositoryException(e.userMessage);

        final localResult = await _local.addTransaction(
          transaction,
          allowOverBudget: allowOverBudget,
        );

        return localResult.fold(
          (failure) => throw RepositoryException(failure.userFacingMessage),
          (write) async {
            final now = DateTime.now().toUtc().toIso8601String();
            final localTx = transaction.copyWith(
              id: write.transactionId,
              updatedAt: now,
            );
            await _cache.upsertExpense(localTx.toJson());
            await _cache.queueOp({
              'op': 'create',
              'local_id': write.transactionId,
              'payload': localTx.toJson(),
              'queued_at': now,
            });
            return ExpenseWriteSyncResult(
              result: write,
              syncedToServer: false,
              queuedOffline: true,
            );
          },
        );
      }
    }, fallbackMessage: AppStrings.expensesSyncFailed);
  }

  Future<Either<Failure, ExpenseWriteSyncResult>> updateTransaction(
    ExpenseTransaction transaction, {
    bool allowOverBudget = false,
  }) async {
    if (transaction.type == 'expense') {
      final validation = await _validateExpense(
        transaction,
        allowOverBudget: allowOverBudget,
        excludeTransactionId: transaction.id,
      );
      if (validation != null) return Left(validation);
    }

    return guardRepository(() async {
      try {
        final remote = await _remote.updateExpense(
          transaction.id,
          _updatePayload(transaction),
        );
        await _applyServerExpense(remote);

        final budget = await _local.getBudgetSnapshot(
          accountId: remote.accountId,
          date: remote.date,
        );

        return ExpenseWriteSyncResult(
          result: ExpenseWriteResult(
            transactionId: remote.id,
            budgetSnapshot: budget,
            budgetMessage: budget == null
                ? null
                : AppStrings.expensesBudgetLinked(
                    budget.spentAmount,
                    budget.budgetAmount,
                    budget.remaining,
                  ),
          ),
        );
      } on ApiException catch (e) {
        if (e.isConflict) {
          final server = await _applyConflictExpense(e);
          if (server != null) {
            final budget = await _local.getBudgetSnapshot(
              accountId: server.accountId,
              date: server.date,
            );
            return ExpenseWriteSyncResult(
              result: ExpenseWriteResult(
                transactionId: server.id,
                budgetSnapshot: budget,
                budgetMessage: budget == null
                    ? null
                    : AppStrings.expensesBudgetLinked(
                        budget.spentAmount,
                        budget.budgetAmount,
                        budget.remaining,
                      ),
              ),
              syncedToServer: false,
            );
          }
        }
        if (!e.isNetworkError) throw RepositoryException(e.userMessage);

        final localResult = await _local.updateTransaction(
          transaction,
          allowOverBudget: allowOverBudget,
        );

        return localResult.fold(
          (failure) => throw RepositoryException(failure.userFacingMessage),
          (write) async {
            final touched = transaction.copyWith(
              updatedAt: DateTime.now().toUtc().toIso8601String(),
            );
            await _cache.upsertExpense(touched.toJson());
            await _cache.queueOp({
              'op': 'update',
              'server_id': transaction.id,
              'payload': touched.toJson(),
              'queued_at': DateTime.now().toIso8601String(),
            });
            return ExpenseWriteSyncResult(
              result: write,
              syncedToServer: false,
              queuedOffline: true,
            );
          },
        );
      }
    }, fallbackMessage: AppStrings.expensesSyncFailed);
  }

  Future<Either<Failure, ExpenseDeleteSyncResult>> deleteTransaction(int id) {
    return guardRepository(() async {
      try {
        await _remote.deleteExpense(id);
        await _cache.removeExpense(id);
        final local = await _local.deleteTransaction(id);
        return local.fold(
          (failure) => throw RepositoryException(failure.userFacingMessage),
          (ok) => ExpenseDeleteSyncResult(deleted: ok),
        );
      } on ApiException catch (e) {
        if (!e.isNetworkError) throw RepositoryException(e.userMessage);

        final local = await _local.deleteTransaction(id);
        return local.fold(
          (failure) => throw RepositoryException(failure.userFacingMessage),
          (ok) async {
            if (ok) {
              await _cache.removeExpense(id);
              await _cache.queueOp({
                'op': 'delete',
                'server_id': id,
                'queued_at': DateTime.now().toIso8601String(),
              });
            }
            return ExpenseDeleteSyncResult(deleted: ok, queuedOffline: ok);
          },
        );
      }
    }, fallbackMessage: AppStrings.expensesSyncFailed);
  }

  Future<Failure?> _validateExpense(
    ExpenseTransaction transaction, {
    bool allowOverBudget = false,
    int? excludeTransactionId,
  }) async {
    final balance = await _local.getAccountBalance();
    var requiredBalance = transaction.amount;
    if (excludeTransactionId != null) {
      final oldRows = await _local.getTransactions(type: 'expense');
      final oldAmount = oldRows.fold(
        (_) => 0.0,
        (rows) {
          for (final row in rows) {
            if (row.id == excludeTransactionId) return row.amount;
          }
          return 0.0;
        },
      );
      requiredBalance = (transaction.amount - oldAmount).clamp(0, double.infinity);
    }
    if (requiredBalance > balance) {
      return ValidationFailure(AppStrings.txInsufficientBody);
    }

    final budget = await _local.getBudgetSnapshot(
      accountId: transaction.accountId,
      date: transaction.date,
      excludeTransactionId: excludeTransactionId,
    );
    if (budget != null &&
        !allowOverBudget &&
        transaction.amount > budget.remaining) {
      return BudgetExceededFailure(
        AppStrings.expensesBudgetExceeded(budget.remaining),
        budgetRemaining: budget.remaining,
      );
    }
    return null;
  }

  Future<void> flushPendingOps() async {
    await SyncFlushLock.run(() async {
      final ops = _cache.getPendingOps();
      if (ops.isEmpty) return;

      final remaining = <Map<String, dynamic>>[];
      for (final op in ops) {
        try {
          final type = op['op'] as String?;
          if (type == 'create') {
          final payload = Map<String, dynamic>.from(op['payload'] as Map);
          payload.remove('id');
          final remote = await _remote.createExpense(payload);
          await _applyServerExpense(remote);
          final localId = op['local_id'] as int?;
          if (localId != null) {
            await _local.replaceLocalId(localId, remote);
            await _cache.removeExpense(localId);
            await _cache.upsertExpense(remote.toJson());
          }
        } else if (type == 'update') {
          final serverId = op['server_id'] as int;
          final payload = Map<String, dynamic>.from(op['payload'] as Map);
          payload.remove('id');
          try {
            final remote = await _remote.updateExpense(serverId, payload);
            await _applyServerExpense(remote);
          } on ApiException catch (e) {
            if (e.isConflict) {
              await _applyConflictExpense(e);
              continue;
            }
            rethrow;
          }
        } else if (type == 'delete') {
          final serverId = op['server_id'] as int;
          await _remote.deleteExpense(serverId);
          await _cache.removeExpense(serverId);
          await _local.deleteTransaction(serverId);
        }
        } on ApiException catch (e) {
          if (shouldRetainPendingOp(error: e, opType: op['op'] as String?)) {
            remaining.add(op);
          }
        } catch (_) {
          remaining.add(op);
        }
      }
      await _cache.setPendingOps(remaining);
    });
  }

  Future<void> _pruneFromServerSnapshot(List<ExpenseTransaction> remote) async {
    final serverIds = remote.map((e) => e.id).toSet();
    final protected = protectedExpenseIdsFromOps(_cache.getPendingOps());
    await _local.pruneExceptServerIds(serverIds, protected);
  }

  Map<String, dynamic> _updatePayload(ExpenseTransaction transaction) {
    final payload = transaction.toJson()..remove('id');
    if (transaction.updatedAt != null) {
      payload['updated_at'] = transaction.updatedAt;
    }
    return payload;
  }

  Future<void> _applyServerExpense(ExpenseTransaction server) async {
    await _cache.upsertExpense(server.toJson());
    await _local.mergeServerExpenses([server]);
  }

  Future<ExpenseTransaction?> _applyConflictExpense(ApiException e) async {
    if (!e.isConflict || e.conflictData == null) return null;
    final server = ExpenseTransaction.fromMap(
      Map<String, dynamic>.from(e.conflictData as Map),
    );
    await _applyServerExpense(server);
    return server;
  }

  List<ExpenseTransaction> _filterCached(
    List<Map<String, dynamic>> cached, {
    required String type,
    String? monthKey,
    int? categoryId,
    bool recurringOnly = false,
  }) {
    return filterCachedExpenseMaps(
      cached,
      type: type,
      monthKey: monthKey,
      categoryId: categoryId,
      recurringOnly: recurringOnly,
    ).map(ExpenseTransaction.fromMap).toList();
  }
}

